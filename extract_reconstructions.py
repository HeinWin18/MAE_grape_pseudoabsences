...
def prepare_model(chkpt_dir, arch='mae_vit_base_patch16'): #mae_vit_large_patch16
    ...
    checkpoint = torch.load(chkpt_dir, weights_only=False, map_location="cpu") #added map_location="cpu" to avoid GPU memory issues
...
def compute_band_range(paths):
    """Per-band min/max over the reference set."""
    mins = np.full(19, np.inf, np.float32)
    maxs = np.full(19, -np.inf, np.float32)

    for p in paths:
        x = process_image(load_image(p)).reshape(-1, 19)
...
def generate_pseudo_abnormal(reconstruction, mins, maxs,
                             n_corrupt_bands=5, delta=1.0):
    """Generate ExDet-style pseudo-abnormal patches.

    NT1: push selected bands outside the training range.
    NT2: shuffle selected bands spatially to create novel combinations.
    """

    corrupted = reconstruction.copy()
    h, w, _ = corrupted.shape

    bands = np.random.choice(
        19,
        size=n_corrupt_bands,
        replace=False
    )
    ...
    for i, b in enumerate(bands):
        ...
        if i % 2 == 0:
            ...
            # NT2: spatially shuffled band
            flat = corrupted[:, :, b].reshape(-1).copy()
            np.random.shuffle(flat)
            corrupted[:, :, b] = flat.reshape(h, w)
...
def write_reconstructions(model_mae, paths, is_abnormal: bool = False, iter_: int = 0):
    ...
    for start_index in tqdm(range(0, len(paths), args.batch_size)):
        ...
        old_reconstructions = get_reconstructions_multi(
            model_mae,
            imgs
        )

        # Generate ExDet pseudo-abnormal reconstructions
        if is_abnormal:
            reconstructions = np.array([
                generate_pseudo_abnormal(
                    recon.copy(),
                    MINS,
                    MAXS
                )
                for recon in old_reconstructions
            ])
        else:
...
parser.add_argument(
    '--exdet',
    action='store_true',
    help='Generate ExDet-style pseudo-abnormal patches.'
)
...
if __name__ == '__main__':
    ...
    for split in ["train", "val"]:
        os.makedirs(
            os.path.join(args.output_folder, split, "normal"),
            exist_ok=True
        )

        if args.exdet:
            os.makedirs(
                os.path.join(args.output_folder, split, "abnormal"),
                exist_ok=True
            )
    ...
    # Compute ExDet reference range
    if args.exdet:
        print("Computing ExDet band ranges...")
        MINS, MAXS = compute_band_range(normal_paths)

    # ---------------------------------------------------------
    # Normal MAE reconstructions
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # ExDet pseudo-abnormal reconstructions
    # ---------------------------------------------------------

    if args.exdet:
        print("Generating ExDet pseudo-abnormal patches...")
